from ..cl_helper_functions import weather_json_to_dataframe
import pandas as pd
...
def test_weather_json_to_dataframe_0():
    ...
    assert result.equals(pd.DataFrame.from_dict([my_dict]))
...
def test_weather_json_to_dataframe_3():
    ...
    assert result.equals(pd.DataFrame.from_dict(my_dict_l))
...
def test_weather_json_to_dataframe_4():
    ...
    df = pd.DataFrame.from_dict(my_dict_l)
    df.set_index('index',inplace=True)
